  ...
  protected void buildEquations(boolean toWorkList, boolean eager) {
    ...
    for (Iterator<? extends T> it = G.iterator(); it.hasNext();) {
      ...
      if (nPred >= meetThreshold) {
        ...
        newStatementIR(getIn(node), meet, rhs, toWorkList, eager);
      }
    }
    ...
    if (functions.hasNodeTransferFunctions()) {
      for (Iterator<? extends T> it = G.iterator(); it.hasNext();) {
        ...
        if (!f.isIdentity()) {
          newStatementIR(getOut(node), f, getIn(node), toWorkList, eager);
        }
      }
    }
    ...
    if (functions.hasEdgeTransferFunctions()) {
      for (Iterator<? extends T> it = G.iterator(); it.hasNext();) {
        ...
        for (Iterator<? extends T> it2 = G.getSuccNodes(node); it2.hasNext();) {
          ...
          if (!f.isIdentity()) {
            newStatementIR(getEdge(node, succ), f, (functions.hasNodeTransferFunctions()) ? getOut(node) : getIn(node), toWorkList,
                eager);
          }
        }
      }
