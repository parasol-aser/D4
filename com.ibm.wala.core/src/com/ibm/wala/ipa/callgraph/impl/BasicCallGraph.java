...
public abstract class BasicCallGraph<T> extends AbstractNumberedGraph<CGNode> implements CallGraph {
  ...
  public void deleteNode(IMethod m_old, Context C, CGNode node)
  {
    Key key = new Key(m_old, C);
    deRegisterNode(key,node);
  }
  ...
  protected final static class Key {
    ...
    @Override
    public int hashCode() {
      return 17 * m.getSignature().toString().hashCode() + C.hashCode();
    }
