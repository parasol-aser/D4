...
public class PointsToMap {
  ...
  public MutableMapping<PointerKey> getPointerKeys(){
    return pointerKeys;
  }
...
//    }
  ...
  public boolean isPointsToMap(PointerKey p){
    if (p == null) {
      throw new IllegalArgumentException("null p");
    }
    int i = pointerKeys.getMappedIndex(p);
    ...
    int repI = uf.find(i);
    Object result = pointsToSets.get(repI);
    if(result instanceof PointsToSetVariable){
      return false;//is com.ibm.wala.ipa.callgraph.propagation.PointsToSetVariable
    }else{
      return true;//is com.ibm.wala.ipa.callgraph.propagation.PointsToMap
    }
  }
  ...
  public void recordImplicit(PointerKey key) {
    ...
    if(key.toString().contains("Lunsound/UnsoundTest1"))
      System.out.println("IMPLICIT     " + key.toString());
