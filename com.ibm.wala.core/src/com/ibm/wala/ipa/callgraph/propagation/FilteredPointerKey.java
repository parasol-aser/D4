...
public interface FilteredPointerKey extends PointerKey {
  ...
  public interface TypeFilter extends ContextItem {
    ...
    boolean delFiltered(PropagationSystem system, PointsToSetVariable lhs, PointsToSetVariable rhs);

    boolean delFiltered(PropagationSystem system, PointsToSetVariable lhs, MutableSharedBitVectorIntSet set);
  ...
  public class SingleClassFilter implements TypeFilter {
    ...
    @Override
    public boolean delFiltered(PropagationSystem system, PointsToSetVariable L, PointsToSetVariable R){
      IntSet f = system.getInstanceKeysForClass(concreteType);
      if(f == null)
        return false;
      IntSet intersection = R.getValue().intersection(f);
      IntSet removed = L.removeSome(intersection);
      if(removed.size() > 0)
        return true;
      else
        return false;
    ...
    @Override
    public boolean delFiltered(PropagationSystem system, PointsToSetVariable L, MutableSharedBitVectorIntSet set) {
      IntSet f = system.getInstanceKeysForClass(concreteType);
      if(f == null)
        return false;
  ...
  public class MultipleClassesFilter implements TypeFilter {
    ...
    private IntSet reverseBits(PropagationSystem system){
      IntSet f = null;
      for(IClass cls : concreteType) {
        if (f == null) {
          f = system.getInstanceKeysForClass(cls);
  ...
  public class TargetMethodFilter implements TypeFilter {
    ...
    private class ReverseUpdateAction implements IntSetAction {
      ...
      private ReverseUpdateAction(PropagationSystem system, PointsToSetVariable L, boolean sense) {
        this.L = L;
        this.sense = sense;
        this.system = system;
      }
