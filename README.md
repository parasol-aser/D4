...
Since we have several large jar files in this github as benchmarks and dependencies, please install Git Large File Storage (LFS) with the command before you gitclone the project:
````git lfs install````.
...
For some variables or methods that you do not want to analyze, you can right-click the variables/methods shown in the Outline or the races/concurrent relations to go to ECHO Bug Choice -> Ignore This Variable. If you want to consider them into the detection later, you can choose Consider This Variable.
