...
//		}
		...
		for (int ctid : ctids) {
			ArrayList<DLPair> DLPairs = engine.threadDLPairs.get(ctid);
			if(DLPairs == null){
				continue;
			}
			for (DLPair pair : DLPairs) {
				if (pair.lock1.equals(check) || pair.lock2.equals(check)) {
					DLPair dllp1 = pair;
					for(Integer tid2: tids){
						if(tid2 != ctid){
							ArrayList<DLPair> DLPairs2 = engine.threadDLPairs.get(tid2);
							for(int j=0;j<DLPairs2.size();j++){
								DLPair dllp2 = DLPairs2.get(j);
...
//				TIDECGModel.bugEngine.addBugsBack(bugs);
	...
	private void processCheckDeadlock(CheckDeadlock job) {
		HashSet<ITIDEBug> bugs = new HashSet<ITIDEBug>();
		ArrayList<DLPair> DLPairs = job.getPairs();
		Set<Integer> tids = job.getTids();
		int tid1 = job.getTid();
		for(int i=0;i<DLPairs.size();i++){
			DLPair dllp1 = DLPairs.get(i);
			for(Integer tid2: tids){
				if(tid2!=tid1){
					ArrayList<DLPair> DLPairs2;
					DLPairs2 = ReproduceBenchmark_remote.engine.threadDLPairs.get(tid2);
//					if(DEBUG)
//						DLPairs2 = Test.engine.threadDLPairs.get(tid2);
//					else
//						DLPairs2 = TIDECGModel.bugEngine.threadDLPairs.get(tid2);
					for(int j=0;j<DLPairs2.size();j++){
						DLPair dllp2 = DLPairs2.get(j);
						HashSet<String> l11sig = dllp1.lock1.getLockSig();
						HashSet<String> l12sig = dllp1.lock2.getLockSig();
